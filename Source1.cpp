...
void setup()
{
	...
	Bootup();//Simulate Bootup process doesnt have to be here this only makes it look cool-idiotic but cool :)
Retry:
	lcd.setCursor(0, 0);
	...
	SentMessage = "ATI"; //Send a test message to see if the ELM327 Chip is responding
	Serial.println("ATI");
	delay(500);
	ReadData();
...
void loop()
{
	...
	if (x > 400 and x < 415)  //down
	{
		if (UpDownMenu < 3)
		{
			UpDownMenu++;
		}
		delay(250); RESETMenuName = 0;
	}
	if (x > 190 and x < 215)  //up
	{
		if (UpDownMenu > 0)
		{
			UpDownMenu--;
	...
	if (UpDownMenu == 0)
	{
		...
		{
			lcd.setCursor(0, 0);
			lcd.print("Engine Readings  ");
			lcd.setCursor(0, 1);
			lcd.print("                      ");
			delay(2000); RESETMenuName = 1;
		}
		if (MenuID == 0)
		{
			lcd.setCursor(0, 0);
			lcd.print("01 Coolant Temp    ");
			lcd.setCursor(0, 1);
	...
	if (UpDownMenu == 3)
	{
		if (RESETMenuName == 0)
		{
			...
			lcd.print("###");
			lcd.setCursor(0, 1);
			lcd.print("\\ELM+Arduino");
			delay(2000);
			RESETMenuName = 1;
		}
		lcd.setCursor(0, 0);
		lcd.print("System Version  ");
	}
...
void Bootup()
{
	lcd.print("Live Autodata");
	for (int i = 0; i <= 1; i++)
	{
		for (int j = 1; j <= 4; j++)// welcome screen
		{
			if (j == 1)
			{
